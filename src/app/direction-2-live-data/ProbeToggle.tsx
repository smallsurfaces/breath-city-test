/**
 * ProbeToggle.tsx — Centre-top probe toggle with a 0-fresh disabled/explained state (pattern 3)
 *
 * Purpose:
 *   Forked from direction-1-mapbox-v2/ProbeToggle.tsx and extended with the brief's third state.
 *   The probe estimates the active parameter at a clicked point by triangulating nearby LIVE
 *   sensors. The toggle has three states:
 ...
 *                              visible BEFORE any click (label + title + aria), so the user knows
 *                              why up front rather than discovering it after clicking the map
 *   This is the pre-click communication the brief requires for the 0-fresh case.
 *
 * Key exports: ProbeToggle
 * External dependencies: react
 *
 * No design decisions: chrome mirrors the v1 toggle; disabled styling uses reduced opacity +
 * muted text + not-allowed cursor (standard disabled affordance). Colours are prototype chrome.
 */
...
export function ProbeToggle({ isActive, isAvailable, onToggle }: Props): React.ReactElement {
  ...
  // Label per state. Disabled communicates the reason BEFORE a click (brief requirement).
          ...
          <>
            <div
              ...
              style={{
                ...
                animation: 'probe-pulse-v2 2.5s ease-out infinite',
                animationDelay: '0s',
                pointerEvents: 'none',
              }}
            />
