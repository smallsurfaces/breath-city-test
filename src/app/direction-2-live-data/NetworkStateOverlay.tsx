/**
 * NetworkStateOverlay.tsx — Calm inline state notes for empty / empty-but-stale / error (pattern 4)
 *
 * Purpose:
 *   Renders the non-marker feedback for three of the four network states. The hard line the brief
 *   draws — and the reason this component exists — is that "loaded, found nothing" and "failed to
 ...
 *                                    an error; markers still render their stale treatment (the map
 *                                    keeps them — this overlay only adds the explanatory note)
 *     - error (fetch failed)      -> visually distinct error affordance (warm/error accent) WITH a
 *                                    Retry button
 *   Loading and ready render nothing here (loading is the header readout's job; ready is just the
 *   map). The component renders null for those so the map is unobstructed.
 *
 * Key exports: NetworkStateOverlay
 * External dependencies: react, ./aqiParameters (PARAMETER_META, ParameterKey), ./useStations
 *   (NetworkStatus)
 *
 * Token note: the error accent uses the BC error/destructive role colour (tangerine,
 *   --bc-semantic-error). It is read at runtime where possible; the empty/stale notes use neutral
 *   prototype chrome (deliberately calm, no semantic-error colour) so they cannot be mistaken for
 *   an error.
 */
      ...
      <div data-slot="network-error" style={containerStyle()} role="alert">
        <div
          style={{
            ...
            WebkitBackdropFilter: 'blur(10px)',
            borderRadius: 16,
            // Distinct from the calm notes: a clear error-coloured top border + left accent.
