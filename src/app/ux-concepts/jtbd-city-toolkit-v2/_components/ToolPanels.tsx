/**
 * ToolPanels.tsx — Visual sketch components for each of the 8 toolkit tools (v2 skin).
 *
 * Purpose
 *   Each component renders a recognisable visual sketch of a real tool (a map, gauge, chart,
 *   table, etc.) using CSS shapes and BC design tokens. No SVG charting libraries — pure
 *   CSS/HTML shapes.
 *
 * V2 change vs v1
 *   The LOCAL aqi helper functions (aqiIndicatorVar, aqiBgVar, aqiLabel, aqiLevelFromValue)
 *   that were duplicated in v1/ToolPanels.tsx lines 30-76 are REMOVED here and replaced with
 *   imports from `@/components/concept` (the shared aqi-palette). The AqiLevel type is also
 *   imported from there rather than from toolkit-data. Everything else — the 8 panel
 *   components, all panel-internal logic, all AQI/severity functional colours, all CSS/SVG
 *   shapes — is preserved verbatim. Do NOT edit `@/components/concept/aqi-palette.ts`.
 *
 *   The local alertBorderColor helper is kept here (it maps a different severity vocabulary —
 *   green/amber/red — not the AqiLevel type, so it has no shared-layer counterpart).
 *
 ...
 *   the Partial badge in ToolCard, data table PM2.5 indicators, API snippet dark-blue/teal.
 *   None of this is decorative; it is the concept's core communication. Light mode only.
 *
...
// V2: AQI helpers and AqiLevel type imported from the shared concept layer.
// The local duplicates that were in v1/ToolPanels.tsx lines 30-76 are removed.
import {
  type AqiLevel,
  aqiIndicatorVar,
  aqiBgVar,
