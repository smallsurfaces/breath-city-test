...
export type AtlasSensor = {
  ...
  placed: boolean
  /**
   * The OpenAQ location id, name and provider this sensor's POSITION came from, or null when it was
   * placed. Provenance only: none of the three is ever rendered, because a station name is the
   * city's to publish and a placed sensor has no name to show.
   */
  openaqLocationId: number | null
  openaqName: string | null
  openaqProvider: string | null
  /**
...
function parseSensor(value: unknown, where: string): AtlasSensor {
  ...
  return {
    ...
    openaqLocationId: asNumberOrNull(field(record, 'openaqLocationId', where), `${id}.openaqLocationId`),
    openaqName: asStringOrNull(field(record, 'openaqName', where), `${id}.openaqName`),
    openaqProvider: asStringOrNull(field(record, 'openaqProvider', where), `${id}.openaqProvider`),
 ...
 */
export function sensorCountsFor(slug: string): { lowCost: number; referenceGrade: number; status: 'placeholder' } {
