 ...
 *     indented column.
 *   Every layout ends with the story's sources, linked (new tab).
 *
 * Accessibility
 ...
 * External dependencies: ./PhotoFigure, ./ChapterLink, ../../_data/chapters (types).
 */
...
/** The story's sources list. Nothing is rendered when there are none. */
      ...
      <ul className="mt-1">
        {story.sources.map((source) => (
          <li key={`${source.label}-${source.url}`}>
            <OutboundLink href={source.url} className="text-sm text-foreground/80">
              {source.label}
            </OutboundLink>
          </li>
        ))}
