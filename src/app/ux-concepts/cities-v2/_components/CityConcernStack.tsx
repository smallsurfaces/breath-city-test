 ...
 *
 * v2: the synchronised copy. SAME structure, content, data, and interactions as v1 —
 * the ONLY difference is skin-level: the two brand-blue accents (the "What residents
 * ask" eyebrow and the per-section number marker) move from the `text-bc-blue` /
 * `text-bc-blue/70` utilities to the inline brand-blue token (a `color-mix` carries
 * the 70% variant). The teal blockquote rule, the modal-on-tap EntryCards, and the
 * honest empty state are all unchanged.
 *
 * Key exports: CityConcernStack
 * External dependencies: EntryCard + concerns-data (both colocated in cities-v2/)
 */
