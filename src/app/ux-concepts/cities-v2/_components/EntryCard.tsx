/**
 * EntryCard.tsx — compact, graphical, scannable entry card for the deck grid (v2)
 *
 * The synchronised v2 copy. SAME structure, content, data, and interaction as v1
 * (tap/click → modal). The ONLY differences are skin-level: the headline-stat figures
 * are capped from text-5xl to text-3xl (the shared ConceptStat scale), and the
 * functional [TK] evidence-gap chip moves from the amber-* Tailwind utilities to the
 * AQI-moderate (amber-family) token. The modal-on-tap interaction is unchanged.
 *
...
function StatViz({ stat }: { stat: EntryStat }) {
  if (stat.kind === "tk") {
    return (
      <div className="flex flex-col items-start gap-1">
        {/* Functional evidence-gap chip — colour moves from the amber-* utilities to
            the AQI-moderate (amber-family) token, consistent across the concept. */}
        <span
  ...
  if (stat.kind === "figure") {
    return (
      <div className="flex flex-col items-start gap-1">
        {/* Headline number capped at text-3xl (was text-5xl) to match the shared
            ConceptStat scale — the card still reads as data-viz at a glance. */}
  ...
  return (
    <div className="flex flex-col items-start gap-1">
      <span className="inline-flex items-center gap-2">
        {/* `from` and arrow stay balanced with the (now text-3xl) `to` value */}
        ...
        />
        {/* Achieved value capped at text-3xl (was text-5xl) — shared ConceptStat scale. */}
        <span className="text-3xl font-bold leading-none tracking-tight text-foreground">
          {stat.to}
        </span>
      </span>
