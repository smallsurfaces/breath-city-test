/**
 * ToolPanels.tsx -- Visual sketch components for each of the 8 toolkit tools
 *
 * Purpose: Each component renders a recognisable visual sketch of a real tool
 * (a map, gauge, chart, table, etc.) using CSS shapes and BC design tokens.
 * No SVG charting libraries -- pure CSS/HTML shapes for v0.
 *
...
/* ------------------------------------------------------------------ */
/*  Shared: AQI level to CSS token mapping                             */
/* ------------------------------------------------------------------ */

/** Maps AQI severity level to the BC semantic indicator token */
function aqiIndicatorVar(level: AqiLevel): string {
...
/** Maps AQI severity level to the BC semantic background token */
function aqiBgVar(level: AqiLevel): string {
  const map: Record<AqiLevel, string> = {
...
/** Maps AQI severity level to a human label */
...
/** Returns the AQI level for a given numeric value */
function aqiLevelFromValue(value: number): AqiLevel {
...
export function MonitoringPanel({ sensors, stationCount }: MonitoringPanelProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Legend */}
      <div className="flex flex-wrap items-center gap-3 text-[11px]" style={{ color: "var(--bc-semantic-muted)" }}>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="relative w-full">
        ...
        {/* Fill bar */}
        <div
          className="absolute top-0 left-0 h-5 rounded-full transition-all duration-500"
          style={{
            width: `${currentPct}%`,
      ...
      {/* Labels */}
      <div className="mt-3 flex items-center justify-between text-[11px]" style={{ color: "var(--bc-semantic-muted)" }}>
        <span>0</span>
        <span>{nationalStandardLabel}: {nationalStandard} ug/m3</span>
        <span>{maxScale}</span>
      </div>
    </div>
  );
...
export function ForecastingPanel({ forecast }: ForecastingPanelProps) {
  return (
    <div className="flex flex-col gap-2">
      {/* Section labels */}
      <div className="flex items-center justify-between text-[10px] font-medium" style={{ color: "var(--bc-semantic-muted)" }}>
            ...
            <div key={i} className="flex flex-1 flex-col items-center gap-1">
              ...
              <div
                ...
                style={{
                  ...
                  border: isToday
                    ? "2px solid var(--bc-semantic-text)"
                    : "none",
                }}
              />
              {/* Day label */}
      ...
      {/* Today marker legend */}
      <div className="flex items-center gap-3 text-[10px]" style={{ color: "var(--bc-semantic-muted)" }}>
        <span className="flex items-center gap-1">
          <span
            className="inline-block h-2 w-3 rounded-sm"
            style={{
              backgroundColor: "var(--bc-semantic-muted)",
              opacity: 0.85,
            }}
          />
          Actual
      ...
      {alerts.map((alert, i) => (
        <div
          ...
          style={{
            ...
            backgroundColor: alert.severity === "green"
              ? aqiBgVar("good")
...
export function AdvocacyPanel({ trendLine, trendStat, trendDirection }: AdvocacyPanelProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        {/* Bars representing trend points */}
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-around px-2" style={{ height: `${chartHeight}px` }}>
          {trendLine.map((val, i) => {
            const barHeight = Math.max((val / maxVal) * chartHeight * 0.85, 4);
            return (
              <div
                key={i}
                className="flex-1 mx-0.5 rounded-t-sm transition-all"
                style={{
                  height: `${barHeight}px`,
