/**
 * BcChrome.tsx — Visual Concept v1 forked BC site chrome (header + footer).
 *
 * Fork origin
 *   Clean fork of src/components/concept/BcChrome.tsx (commit 09839c6 / tag
 *   wireframe-lock-2026-05-26). The fork exists so future visual evolution on the BC site
 *   chrome inside this sandbox cannot leak into the wireframe-locked UX concepts that depend
 *   on the shared original. First-deploy render is identical to the shared version.
 *
 *   The local BcChromeConfig type comes from ./bc-chrome.config (also forked), so the
 *   visual-concepts namespace has no remaining imports from @/components/concept.
 *
 * Purpose (carried forward from shared)
 *   ONE reusable header + footer that recreates breathecities.org's chrome for the visual
 *   concept, so its pages can be shown embedded in BC's real site IA and the prototype is
 *   NAVIGABLE between concept home and sub-pages. NOT pixel-perfect — an approximation of BC's
 *   look: brand blue/teal, clean sans, airy whitespace. Placeholder logos only; we never
 *   reproduce BC's real assets.
 *
 *   The visual concept's per-route nav (which slots are LIVE, where the brand mark points)
 *   is passed in as a BcChromeConfig (see ../roadmap-chrome.config.ts). BcHeader takes that
 *   config; BcFooter is static content and takes no props.
 *
 ...
 *
 *   Back-to-hub is NOT rendered here — the forked PrototypeHeader (rendered ABOVE BcHeader in
 *   the visual concept layout) owns the sole back-to-hub + comment tooling. BcChrome is the
 *   SITE nav; PrototypeHeader is the TOOLING bar. Both coexist: PrototypeHeader on top,
 *   BcHeader below.
 *
 * Behaviour preserved verbatim from the shared chrome:
 ...
 *   `text-foreground`, `text-muted-foreground`, `bg-muted`). All BC BRAND colours are applied
 *   via inline `style` with `var(--bc-*)` tokens — no `text-bc-*`/`bg-bc-*`/`border-bc-*`
 *   utility classes, no hardcoded hex. Light mode.
 *
...
/**
 * One live desktop nav link. Reads as foreground at rest and brand blue on hover/focus. The
 * hover colour uses the shadcn-bridged `primary` token (globals.css maps --primary →
 * --bc-semantic-brand = brand blue), so it is brand blue via a bridged semantic — NOT a
 * `*-bc-*` utility and NOT a JS handler (so this works whether the host page is a server or
 * client component).
 */
function NavLink({
  label,
...
/**
 * The site header — brand mark (left) → primary nav → "Join us" CTA, with a mobile hamburger
 * that opens a full-screen overlay. Driven entirely by `config`: the logo links to
 * `config.logoHref`, and each `config.nav` item renders live (a NavLink) or inert (a
 * non-clickable muted span) based on `href !== '#'`. No prototype/disclaimer bar is rendered
 * here — the single visual-concept disclaimer is owned by the forked PrototypeHeader above
 * this in the visual concept layout (which also owns the sole back-to-hub), so the prototype
 * shows exactly one disclaimer row. BcHeader is the SITE nav only.
 */
  ...
  return (
    <>
      {/* No prototype/disclaimer bar here. The wireframe framing is single-sourced in the
          forked PrototypeHeader above, so BcHeader renders only BC's recreated site chrome —
          exactly one disclaimer row across the whole prototype. */}

      {/* BC-style header — sticky + backdrop blur (preserved from the shared chrome). */}
      ...
      {/* Mobile nav overlay — full-screen, dark navy background (preserved from the shared chrome). */}
      {menuOpen && (
        <div
